...
void SignalRecevoir(int signal, siginfo_t * info, void * context)
{
	...
	for(i = 0; i<NB_BATEAUX && bateau == NULL; i++)
	{
	...
	if(bateau != NULL)
	{
		...
		if(coule == VRAI)
		{
			fprintf(stdout, "Bateau : %i '%c' - Touché\n", pid, bateau->marque);
		...
		else
		{
			if(bateau != NULL && marqueDetruite != bateau->marque) // Risque de segfault sinon
			{
				...
				if(success == VRAI)
				{
					/* Ne pas tirer, envoyer un sigkill au bateau qui meurt */
	...
	{
		if(nbBateauxTotal < nbBateauxMax) // Place un bateau si le nombre maximal de bateau n'est pas atteint
		{
		...
		else // KILL le processus bateau s'il ne peut pas être ajouté à la mer
		{
